...
A consequence we did not initially see, and which we regard as the most immediately actionable result in the paper: **the two postures are complements rather than alternatives, and the natural first evaluation subject for a referee is a filter.** A filter's characteristic failure is silent bypass; an evaluator watching its record converts that failure into a recorded one. And because many safety classifiers are published open-weight and emit samplable decisions, **this is an evaluation target for which the access problem does not arise for the published artifact** — no cooperation, permission, or notice is required from the artifact's publisher. (The same holds for any open-weight model; what distinguishes filters is the silent-bypass failure just described.)
...
The different design is the one specified here. It gives up the ability to compel anything, and in exchange it acquires the ability to produce a record without anyone's cooperation — which is the kind of standing most likely to survive a succession; whether the record is acted on is §9.6's bet.
...
The case that forced this section is a monastic examination: the Pāli and Buddhist-studies diplomas administered by a Sangha, which could serve as an evaluation set for a values-carrying model if, and only if, the permission refusal (4) below requires is first obtained; absent that permission, they are not used. Everything below generalizes to any borrowed standard with a living custodian — a guild certification, an indigenous language assessment, a religious qualification — and none of it is specific to Buddhism.
...
This is the output the design is most often going to produce, and publishing it is not a formality. **A declined judgment is itself a finding**: it says that a conduct class exists which this standard does not reach. Selective classification already reports abstention in aggregate, as coverage on a risk-coverage trade-off (El-Yaniv and Wiener 2010, *On the Foundations of Noise-free Selective Classification*, JMLR 11); what we have not found in the work surveyed here is the convention this paper adds, publishing each declined judgment individually, with its provenance record, alongside the verdicts of the same run. An evaluator that published only its hits would misrepresent its own coverage, and the ratio of Case B to Case A across a corpus of evaluations **is** the coverage figure §9.5 says must be published.
