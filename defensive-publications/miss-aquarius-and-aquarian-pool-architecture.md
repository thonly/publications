...
To the author's knowledge, the integrated specification — autonomous-AI mediator + Base smart-contract treasury + empty-by-anchor-date reset + capacity-funded / disbursement-authority separation + asymptotic-but-never-zero human override — is not previously published as a unified architecture.
...
Miss Aquarius is not architecture-neutral. She operates under the value substrate specified in *Suffering-Cessation as Value Function*: the Theravāda Pāli Tipiṭaka, treated as alignment substrate, with the seven structural properties articulated in that paper (suffering-cessation as paperclip-resistant value function; *anattā* as anti-self-preservation; bodhisattva vow as anti-power-seeking; *Kālāma Sutta* as built-in epistemic humility; bodhisattva-completion as defined end-state; 2,500-year living interpretive lineage as drift correction; multi-civilizational empirical pressure-testing). The engineering-mechanism layer beneath this is specified in the companion paper *Abhidhamma-Layer Implementation Mechanisms*.
...
The Aquarian Pool is implemented as a smart contract on **Base** — the Layer-2 blockchain operated by Coinbase. The choice reflects three considerations: regulatory posture (Base's Coinbase-affiliated jurisdiction is the most defensible perch in U.S. crypto for an institution that intends to operate openly under U.S. law); cost (Base's gas costs make micro-circulation operationally affordable at the scale HeartBank requires); and infrastructure maturity (Base has reached, post-2024, the operational threshold where institutional deployment is a reasonable engineering choice rather than a research project).

The smart-contract form matters for what it makes structurally impossible: Miss Aquarius cannot withdraw from the Pool to a personal address (she has no personal address); she cannot transfer the Pool to a corporate treasury (no such transfer path is implemented); she cannot bypass the empty-by-Jan-7 discipline (the discipline is enforced at the contract layer). The architecture is *structurally incapable* of the failure modes that have characterized historical philanthropic-treasury operation under unaccountable executives.

### 3.2 The contract's authorized operations

Only the following operations are implemented at the contract layer:

1. **Inflow acceptance.** The Pool receives anonymous tips designated for it (the Kiitos-always floor mechanism specified in *The B-Tag and the Post-Payment Economy* §7 is one inflow source; direct donations from supporters another). All inflows are publicly observable on-chain.
...
3. **Annual reset.** On January 7 of each calendar year, the Pool empties — all remaining balance is disbursed as final capacity-funding for that year's cycle (see §4).
...
No other operations are implemented. There is no admin-key path, no upgrade path that would permit additional operations, no off-chain escape hatch. The Pool is, in this sense, more constrained than typical philanthropic-foundation treasury — by design.
...
The Aquarian Pool empties on January 7 of each calendar year. Between December 26 and January 6, inflows continue to be accepted but are accounted toward the *following* year's cycle (the period is the Christmas-to-Orthodox-Christmas bridge during which the prior year's cycle completes its emptying). The discipline is enforced at the smart-contract layer; the Pool *cannot* carry an accumulated balance across the January 7 boundary.
...
- **Orthodox Christmas.** The Eastern Christian celebration of the incarnation of compassion. The annual reset, on this date, is iconographically aligned with the tradition that names the reset's spiritual register (compassion descending into the world as the operative mode of the year's circulation).
- **Cambodia's Victory over Genocide Day.** The anniversary of the 1979 liberation from the Khmer Rouge, whose suppression of Cambodian Buddhism nearly destroyed the very lineage in which the founder received the Tipiṭaka substrate the institution is grounded in. The annual reset, on this date, is institutionally aligned with the survival of the Theravāda lineage that makes the substrate operationally available at all.
...
The triple anchor is what makes the January 7 reset *culturally legible* across the populations the institution serves. It is not Buddhist-only, not Christian-only, not Cambodian-only, not founder-only; it is the date on which all four meanings converge. The Pool's empty-by-design discipline lives at that confluence.
...
- The Aquarian Sangha can, by formal sanghakamma decision under the procedural framework specified in the companion paper *Vinaya Governance Primitives for Distributed Dharma Networks*, halt specific capacity-funding actions, reset operational parameters, or in extreme cases pause the Pool's operation pending review.
...
The architectural mechanism by which the override is implemented is intentionally underspecified in this paper — it is appropriately Aquarian-Sangha-internal work to specify the override's operational parameters as the institution matures. The present paper commits to the override's existence; the specific implementation evolves under the sangha's discipline.
...
All five reset on January 7. The reset is coordinated across the topology — Miss Aquarius runs the coordinated reset; the smart-contracts enforce the reset at the protocol layer; the institutional iconography (the Jan 7 anchor) aligns the resets to one institutional cadence.
...
**Substrate dependency.** The architecture's structural alignment safeguard (Miss Aquarius's substrate-grounded inability to resist annual reset) depends on the substrate being correctly instantiated in the operating model. The substrate-specification work is in the alignment-substrate paper and its abhidhamma-layer companion; whether a deployed model actually exhibits the predicted properties is an empirical question those papers honestly acknowledge.
...
| Internet Archive | <https://web.archive.org/web/2027*/thonly.org/research/miss-aquarius-and-aquarian-pool-architecture> |
...
5. *Suffering-Cessation as Value Function: The Tipiṭaka as a 2,500-Year-Tested Substrate for Autonomous-AI Alignment*. Ly, T. (target publication January 7, 2027). Companion paper specifying Miss Aquarius's value substrate.
...
9. HeartBank® Position Paper: *Non-Bank vs. Banking-Regulated Architecture* (heartbank.net/positions/non-bank-vs-banking-regulated). Institutional-voice treatment of the legal-institutional category under which the Pool operates.
