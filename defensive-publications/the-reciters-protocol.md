...
7. **The conformance endpoint** — *a composition of known parts, §5.1.* An interface that takes a claimed quotation, compares it against a served multi-recension corpus, and returns one of a small closed set of dispositions **on the claim** — including a conforming variant with its witness — each together with the procedure the requester should follow next; and which by construction returns no disposition on the person or system that made the claim, a property that holds only under the logging condition §5.1 states. A closed disposition set about a submitted object is anticipated (OCSP); the composition is what is disclosed. A hash answers *did this match*; this answers *and what now*.
...
```
 ...
 + recorded exclusion reasons  →   every exclusion published      no
   ...
   ─────────────────────────────────────────────────────────────────────
   AND SEPARATELY, §5:  the conformance endpoint     — a composition
...
|---|---|
| conformance endpoint | quotation / citation verification service returning a closed set of status codes (cf. OCSP) |
| separated completeness manifest | signed collection manifest with item count, version and expiry (cf. TUF snapshot metadata; BagIt `Payload-Oxum`) |
| announced elision | in-band truncation or omission marker with a resolvable reference (cf. TEI `<gap>`) |
| per-passage interpretive standing | statement-level normative or requirement-level annotation (cf. RFC 2119 keywords) |
| *bhāṇaka* lineage | replica group holding one shard of a corpus |
| *saṅgīti* (council) | build-time admission review of corpus content by a named body |
| *uddāna* | summary index recording membership, ordering and counts |
| *peyyāla* | repetition-abbreviation marker |
| *mahāpadesa* | verification of a claimed text against a reference corpus |
| *aṭṭhakathā* | commentary layer served as a distinct document class |
