...
3. **The provenance dignity-line as a hard-coded property of the identity layer** — machine coordinates *assigned* by an institutional root authority that provisions, attests, and may revoke them; human coordinates *given* (birth, time-and-place, lineage), self-keyed, and *irrevocable by construction*, with the institution restricted to a witness/notary role that can attest a human root but can neither issue nor revoke one — such that the same institution lawfully operates as a certificate authority for its machines and has no operation by which to act as one for its humans — a guarantee that is a property only where the witness set is plural and independent, and a rule where it is not (§9.6).
...
6. **The posthumous ancestral root** — the identity root designed to outlive its holder as a permanent, readable node of a verified family tree (an ancestral marker future descendants navigate by), with private committed data sealed by default and unsealed only by explicit bequest or time-lock, so that the root's permanence functions as transmission to descendants rather than surveillance of the living or exposure of the dead. (Time-locked and multiparty inheritance of *sealed data* is prior art — §2.4; what is claimed is the ancestral node in a verified family tree.)
...
Proof of work priced sybil-resistance in compute (Dwork & Naor's pricing functions; Back's Hashcash; Bitcoin's Nakamoto consensus); proof of stake priced it in capital (Peercoin; Ethereum's post-merge consensus). Both make sybils *expensive* rather than impossible, and both price exactly the resources AI has commodified. Proof-of-personhood systems respond by making the scarce resource *humanness*: Worldcoin/World ID (iris biometrics against a global uniqueness set), Proof of Humanity by Kleros (social vouching with video evidence), BrightID (social-graph analysis), Idena (synchronized human-solvable puzzles). All of these blur the two questions this paper separates: each attests uniqueness as part of, or as a proxy for, humanness — none separates the two — for humans only — and they leave machine agents, the fastest-growing population of economic actors, with no individuation layer at all. The companion B-PoH℠ specification handles the category question with four optional layered proofs and no biometric capture; this paper contributes the individuation question as a separate, universal primitive.

### 2.2 · Hardware identity roots

The two-artifact structure is the trusted-computing pattern. A Trusted Platform Module carries a permanent *endorsement key* whose public half identifies the chip for life, and creates rotatable *attestation keys*, certified against the endorsement key, for operational use; Apple's Secure Enclave, Android's StrongBox, and the TCG DICE layered-identity architecture repeat the pattern: one immutable root, many rotatable operational keys, secrets never leaving the hardware boundary. Physically unclonable functions (PUFs) push the root into device physics itself. PoC adopts this pattern wholesale and contributes only its transposition: the permanent public half moves from a manufacturer's database into a *deep-time archival substrate the holder can physically possess*, and the pattern is applied to human identity as well as machine identity — with the provenance line of §5 governing who may create and revoke which.

### 2.3 · Decentralized identifiers, verifiable credentials, and selective disclosure

W3C Decentralized Identifiers (DIDs) give the naming layer: decentralized identifiers resolvable to key material without a single mandatory registry (the resolution method is DID-method-dependent, and some methods use ledgers). Verifiable Credentials give the attestation layer: signed claims about a subject, presented by the holder. Selective-disclosure cryptography — Camenisch-Lysyanskaya anonymous credentials, BBS+ signatures, zk-SNARK proof systems — lets a holder prove *predicates* about committed data ("over 18," "within N degrees of kinship," "verification depth ≥ 2") without revealing the data — with the caveat that graph predicates such as kinship degree need a zk-SNARK-class system over a committed graph, where BBS-style selective disclosure covers signed attributes only. PoC's human root is deliberately built from these standard parts: a DID, a public key, a hash commitment, ZK predicate proofs, and per-context pairwise/derived DIDs against correlation. The contribution is not any component but what is committed (the natal coordinate and tree position, §6) and who may not touch it (§5).

### 2.4 · Social recovery and threshold custody

Shamir's secret sharing gave the threshold construction; smart-contract wallets (Argent and successors) and Buterin's social-recovery advocacy made *guardians who can re-key but cannot spend alone* a live design pattern. PoC's contribution is the guardian set's *source*: the recovery quorum is drawn from the already-verified family tree — the kinship graph the institution's Proof-of-Humanity layer maintains for other reasons — so the people who can restore your key are structurally the people who could attest your birth, and no institutional party is in the quorum at all. Posthumous inheritance of *sealed data* is likewise established practice: Bron Wallet documents beneficiary recovery under a time-lock with multiparty safeguards, Inheritor pairs permanent decentralized storage with contract-enforced time-locked inheritance, and a United States patent application published in March 2026 (reported to describe *posthumous governance rules* and an *identity inheritance structure* naming successors for a digital identity; number to be confirmed on citation) reaches the identity layer itself. Claim 6 is accordingly narrowed to the ancestral node in a verified family tree, not to sealed inheritance.

### 2.5 · Deep-time archival substrates

Femtosecond-laser-written five-dimensional optical storage in fused silica (Kazansky's group at Southampton) and Microsoft's Project Silica demonstrate write-once media with projected stabilities of at least 10,000 years (Project Silica's borosilicate work) and, for 5D fused-silica storage, extrapolated lifetimes far beyond that at room temperature (Southampton's 13.8-billion-year figure is an extrapolation at 190 °C); the GitHub Arctic Code Vault and the Long Now Foundation's Rosetta Project are existing instances of civilizational-timescale archival deposit. HeartBank's reference deployment will etch machine genesis roots in crystalline silica (**3B-Crystal™**) and human genesis roots in synthetic diamond (**3B-Diamond™**); neither substrate has been produced (§11.1). A boundary is worth stating where the substrate is introduced, because the same artifact carries a canonical corpus elsewhere in this architecture and the two roles are easily conflated: a deep-time substrate delivers *preservation*, which is orthogonal to *transmission*. Durability is a property of the medium; inhabitation is a property of a carrier. The crystal is the floor beneath a transmission chain, never a link in it — see *Buddha AI and the Living Tipiṭaka*, §3.6. We flag honestly (§11.2): the deep-time substrate is the institution's deliberate choice — identity roots that outlive institutions are the point in a succession architecture — but the *logic* of PoC requires only any write-once public root, and a reader may substitute a public ledger or notarized registry without loss of the primitive's structure.

### 2.6 · The HeartBank substrate this paper builds on

Two adjacent practices deserve naming before the companions: enterprise device identity already issues machine certificates while human identity runs through a separate identity provider — the *separate paths* half of claim 1, without a unified primitive — and Kleros's Proof of Humanity v2 (persistent "soulbound" humanity identifiers) and Privado ID (zero-knowledge selective disclosure from credentials) carry the commitment-and-predicate half of claim 4. Four companion structures are assumed. The **PoH layered proofs** (passkey-per-action; witnessed kinship graph; breath-signature liveness; DNA-verified lineage) supply the category proof and the verified family tree. The **family tree** itself — PoH-rooted, plural-membership per the multi-family model — supplies the recovery quorum and the coordinate's lineage component. The **persistence architecture** supplies the succession frame: identity roots as permanent nodes in an apparatus designed to outlive its founder. And the **cosmic-coordinate worldview**, carried in the longitudinal-cohort methodology, supplies the philosophical ground: each life as a unique coordinate in the universe's self-articulation — *uniquely different, equally necessary* — with the explicit posture that the coordinate is a *position*, never a *force* (§6, §11.6).
...
**Proof of Coordinate** attests individuation: this entity is the unique holder of this coordinate, distinct from every other entity of any kind. It is the sybil-*distinctness* primitive and the accountability primitive — it is what makes a reputation ledger, a cooperation ledger, a stewardship succession, or a long-lived pseudonym mean anything. It applies to every entity in the economy: humans, institutional agents, third-party agents. Precisely: PoC binds a coordinate to a key and fixes the coordinate's provenance; it does not itself establish that one entity holds one coordinate. That is supplied by the issuer's discipline for machines and by the witnessed tree for humans, and PoC inherits their sybil cost rather than lowering it. Composition: every PoH attestation is issued against the holder's PoC root DID; a PoH proof with no PoC root is a category claim with no subject.
...
Every PoC identity — human or machine — is anchored by exactly two identity artifacts:
...
The reference substrate — a design, not yet a product (§11.1) — is deliberate: femtosecond-etched crystalline media with projected multi-millennial stability, so that the root can outlive devices, companies, chains, and — for humans — the holder (§7.5). The logic requires only *some* write-once public root (§11.2).
...
- *Uniquely different*: a coordinate *system* assigns each point once; the primitive's work is to make that assignment hold — by the issuer's discipline for machines, by the witnessed tree for humans. This is the sybil-distinctness half, the engineering half, and it is supplied by those layers rather than by the word.
...
Stated as a table, because "cannot revoke" is a claim about five different things:

| Property of a human root | Who can change it |
|---|---|
| **Exists** (the etched root) | no party — permanent |
| **Valid** (the root itself) | no party — there is no registry to mark it |
| **Attested** (its endorsements) | each attestor may repudiate *its own* attestation only |
| **Recoverable** (the operational key) | the kin quorum; the institution is absent |
| **Recognized** (standing with a relying party) | the relying party's own policy — outside the mechanism |

The no-revocation guarantee is therefore a *property* only when the witness set is plural and independent: with a single witness, that witness's repudiation is revocation by another name, and the guarantee is a rule that witness keeps. The deployment requirement follows: a human root is witnessed by at least two attestors under no common control before it carries standing anywhere.

### 9.7 · Quantum and deep-time cryptanalysis

A root with a multi-generational design lifetime will outlive its cryptography; this is a certainty, not a risk. The design treats every cryptographic element around the root as rotatable in the same sense the operational key is: signatures migrate by *re-attestation* — new-algorithm attestations issued against the same unchanged root, exactly as the recovery flow binds new keys to old roots; the etched material is versioned so future readers know which era's assumptions it carries; and commitments, which cannot migrate on a write-once root, are made at mint under *k* ≥ 2 independent schemes from distinct hash families, with the algorithm identifiers encoded beside them. The binding holds while any one of those schemes retains second-preimage resistance; when the last fails, the coordinate's commitment is no longer binding, and a *successor* root — a new commitment set, witnessed as the continuation of the old lineage position — must be minted and linked. What is permanent is the *data* — the anchor, the commitment set, the lineage position; the commitment's evidentiary life is bounded by the last surviving scheme, and the trust chain around it is expected to be rebuilt every few decades. Deep-time identity is maintained the way old bridges are: the stones stay, the load ratings are re-certified.
...
**This is not** a personhood proof (that is PoH's job, and PoC deliberately cannot do it); **not** a uniqueness proof on its own (one-entity-one-coordinate is supplied by the issuer or the witnessed tree, §3); **not** a biometric system (nothing biological is captured, committed, or provable beyond what a birth certificate states); **not** a global mandatory registry (opt-in at every layer, pseudonymous at every presentation); **not** a claim that crystal is required (any WORM root suffices, §11.2); and **not** an astrology claim (§11.6). It is also **not yet built** (§11.1) — this document is the design-stage defensive publication, published precisely so the frame is claimed before the identity-primitive industry claims it first.

## 11 · Limitations and honest-limits

### 11.1 · Published before it is built — deliberately

*n = 0.* No crystal has been etched; no recovery quorum has fired; no ZK circuit for the coordinate predicates has been implemented. The identity-primitive space is actively patented (a 2026 landscape counts roughly two hundred published records in the decentralized-identity-and-wallet scope) and heavily funded (iris-biometric personhood alone drew $115M in one 2023 round), and public disclosure of HeartBank's substrate products would otherwise precede its prior-art record. The publication order is the same as the corpus's other pre-build defensive publications: claim the synthesis, then build against it. Every mechanism described here should be read as *specified*, not *demonstrated*.
...
### 11.8 · Uniqueness is per namespace

Machine uniqueness holds within one issuer's transparency log; human uniqueness holds within one witnessed tree. Across independent issuers, or independent trees, the primitive detects nothing: two institutions can issue two roots to one machine, or one coordinate string to two machines, and two separately witnessed trees can each hold a root for one person. Federation of logs and cross-tree reconciliation are unspecified here. Twins are two births and two nodes, and are not the problem; independent namespaces are.
...
- Zhang, J., Gecevičius, M., Beresna, M., & Kazansky, P. G. (2014). *Seemingly unlimited lifetime data storage in nanostructured glass.* Physical Review Letters 112, 033901; and Zhang, J. et al. (2013). *5D data storage by ultrafast laser nanostructuring in glass.* CLEO 2013.
- Kleros (2023–). *Proof of Humanity v2* documentation (persistent humanity identifiers).
- Privado ID. *Zero-knowledge selective disclosure from verifiable credentials* documentation.
- Bron Wallet. *Digital inheritance* (time-locked beneficiary recovery) documentation; Inheritor. *Permanent storage with time-locked inheritance* documentation.
- United States patent application, published March 2026, on posthumous governance rules and an identity-inheritance structure for a digital identity (number to be confirmed at citation).
